...
//        ddayTitleTextField.addTarget(self, action: #selector(textFieldDidChange(_:)), for: .editingChanged)
    ...
    @objc func keyboardWillShow(notification: NSNotification) {
        if let keyboardSize = (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue {
            if ddayTitleTextField.isFirstResponder {
                // 뷰를 올릴 필요가 있는지 결정하기 위해 키보드의 높이와 텍스트 필드의 위치를 계산
                let bottomOfTextField = ddayTitleTextField.convert(ddayTitleTextField.bounds, to: view).maxY
                let topOfKeyboard = view.frame.height - keyboardSize.height

                // 키보드가 텍스트 필드를 가리는 경우 뷰를 올림
                if bottomOfTextField > topOfKeyboard {
                    view.frame.origin.y = 0 - (bottomOfTextField - topOfKeyboard)
                }
            }
        }
    }
